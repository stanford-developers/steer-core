...
class PlotterMixin:
    ...
    def plot_breakdown_sunburst(
        ...
        **kwargs
        ...
        def _prepare_sunburst_data(data: Dict[str, Any], parent_id: str = "", 
                                 current_path: str = "") -> Tuple[List[str], List[str], List[str], List[float]]:
            ...
            for key, value in data.items():
                ...
                if isinstance(value, dict):
                    ...
                    nested_ids, nested_labels, nested_parents, nested_values = _prepare_sunburst_data(
        ...
        ids, labels, parents, values = _prepare_sunburst_data(breakdown_dict, parent_id="")
        ...
        for i, (label, value) in enumerate(zip(labels, values)):
            if label == root_label:
                ...
                unit_str = f" {unit}" if unit else ""
                hover_text.append(f"<b>{label}</b><br>{value:.2f}{unit_str}<br>{percentage:.1f}% of total")
