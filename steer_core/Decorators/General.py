...
def recalculate(*method_names: str, requires: Optional[Dict[str, Callable]] = None):
    ...
    ----------
    *method_names : str
        One or more method suffixes. For each name, ``self._calculate_<name>()`` is
        called in order after the wrapped function executes.
    requires : dict, optional
        Extra attribute guards. Keys are attribute names (e.g. ``"_mass"``), values
        are callables that receive the attribute value and must return ``True`` for the
        recalculation to proceed.  All guards must pass.
